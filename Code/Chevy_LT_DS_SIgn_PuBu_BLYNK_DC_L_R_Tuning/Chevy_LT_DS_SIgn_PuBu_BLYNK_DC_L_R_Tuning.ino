...
// There is a difference of approx. 30 between left and right motor when running on battery !!!!
int LEFT_SPEED_F = 120; // 70 is min. to start rotating on AC power
int RIGHT_SPEED_F = 90; // 60 is min. to start rotating on AC Power
int LEFT_SPEED_T = 150; // 70 is min. to start rotating on AC power
int RIGHT_SPEED_T = 120; // 60 is min. to start rotating on AC Power
...
void forward() {
  analogWrite(ENA, LEFT_SPEED_F);
  analogWrite(ENB, RIGHT_SPEED_F);
...
void turnLeft() {
  analogWrite(ENA, LEFT_SPEED_T);
  analogWrite(ENB, RIGHT_SPEED_T);
  digitalWrite(IN1, LOW); // IN1 = 0
...
void loop() {
  ...
  */
  ...
  // Halt Car for 10 seconds, unless hardware or Virtual (BLYNK) push-button is pushed
  else if (!(digitalRead(LT_L)) && !(digitalRead(LT_M)) && !(digitalRead(LT_R))) { 
    GreenLEDsOn();
    delay(500); // Wait 500ms before halting the car
    myServo.write(servoCenterPos);
    stop_car();
    museum_timer = 10000; // Halt Car for 10 seconds
