...
// set car speed
int CAR_SPEED = 80; // 255 is Max. , Min. is 70
...
void loop() {
  ...
  */
  ...
  // Also Turn on the Yellow LED !!!!
  while (cm < 20.0) {
    OLED_stopcar();
    stop_car();
    YellowLEDsOn();
    measure_distance();
    Blynk.run(); // Otherwise BLYNK will crash while in this loop
    timer.run(); // Otherwise BLYNK will crash while in this loop
  }
